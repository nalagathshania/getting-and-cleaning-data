if (!("reshape2" %in% rownames(installed.packages())) ) {
  ...
  activity_lab <- read.table("D:/datasciencecoursera/UCI HAR Dataset/activity_labels.txt",col.names=c("activity_id","activity_name"))
  
  ## Read the dataframe's column names
  feat <- read.table("D:/datasciencecoursera/UCI HAR Dataset/features.txt")
  feat_names <-  feat[,2]
  
  ## Read the test data and label the dataframe's columns
  testdata <- read.table("D:/datasciencecoursera/UCI HAR Dataset/test/X_test.txt")
  colnames(testdata) <- feat_names
  
  ## Read the training data and label the dataframe's columns
  traindata <- read.table("D:/datasciencecoursera/UCI HAR Dataset/train/X_train.txt")
  colnames(traindata) <- feat_names
  
  ## Read the ids of the test subjects and label the the dataframe's columns
  test_sub_id <- read.table("D:/datasciencecoursera/UCI HAR Dataset/test/subject_test.txt")
  colnames(test_sub_id) <- "subject_id"
  
  ## Read the activity id's of the test data and label the the dataframe's columns
  test_act_id <- read.table("D:/datasciencecoursera/UCI HAR Dataset/test/y_test.txt")
  colnames(test_act_id) <- "activity_id"
  
  ## Read the ids of the test subjects and label the the dataframe's columns
  train_sub_id <- read.table("D:/datasciencecoursera/UCI HAR Dataset/train/subject_train.txt")
  colnames(train_sub_id) <- "subject_id"
  
  ## Read the activity id's of the training data and label 
  ##the dataframe's columns
  train_act_id <- read.table("D:/datasciencecoursera/UCI HAR Dataset/train/y_train.txt")
  ...
  ## the dataset according to  the average of each variable 
  ##for each activity and each subjec
  mean_data <- dcast(data_melt,activity_id + activity_name + subject_id ~ variable,mean)
  
  ## Create a file with the new tidy dataset
  write.table(mean_data,"D:/datasciencecoursera/UCI HAR Dataset/tidy_movement_data.txt",row.names = FALSE)
